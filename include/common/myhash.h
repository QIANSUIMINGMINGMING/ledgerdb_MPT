...
namespace CPUHash
{
    ...
    inline void keccakf(uint64_t st[25], int rounds)
    {
        int i, j, round;
        uint64_t t, bc[5];
    ...
    inline void keccak1600(const uint8_t *in, size_t inlen, uint8_t *md)
    {
        keccak(in, inlen, md, sizeof(state_t));
    }

    inline void calculate_hash(const uint8_t *input, int input_size, uint8_t *hash)
    {
